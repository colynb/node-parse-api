...
Parse.prototype = {
  ...
  findMany: function (className, query, mods, callback) {
    if (!mods.skip) mods.skip = 0;
    if (!mods.limit) mods.limit = 100;
    if (!mods.order) mods.order = null;
    parseRequest.call(this, 'GET', '/1/classes/' + className, {skip: mods.skip, limit: mods.limit, order:mods.order, where: JSON.stringify(query) }, callback);
  },
